...
class Limit(_PluginBase):
    ...
    def _set_torrent_speed(self, service: ServiceInfo, _hash: str, _speed: int = None):
        if not service or not service.instance:
            return
        downloader_obj = service.instance
        # 统一通过 MoviePilot 封装的 change_torrent 设置上传限速，单位均为 KB/s
        # （qB 原始 API torrents_set_upload_limit 单位为字节/秒，封装层会自动 ×1024；
        #   直接调原始 API 会导致 qB 限速偏小 1024 倍，故 qB/Transmission 都走 change_torrent）
