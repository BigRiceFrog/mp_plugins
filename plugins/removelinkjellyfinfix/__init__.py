...
class FileMonitorHandler(FileSystemEventHandler):
    ...
    def on_deleted(self, event):
        ...
        if event.is_directory:
            ...
            # 某些媒体服务器（例如 Jellyfin）删除媒体时，文件系统监控器
            # 可能只产生一个“目录删除”事件，而不会为目录内每个文件产生
            # DELETE 事件。RemoveLink 原本只在文件 DELETE 事件中调用
            # handle_deleted()，因此这种情况下 file_state 中残留的文件记录
            # 不会进入延迟删除队列，硬链接也就无法清理。
            #
            # 这里利用 file_state 中保存的 dev/inode 信息，把已删除目录下
            # 仍在监控状态中的文件逐个按“文件删除”处理。若文件级 DELETE
            # 事件随后到达，handle_deleted() 会因为记录已经从 file_state 移除
            # 而自动跳过，因此不会重复加入删除队列。
...
class RemoveLinkJellyfinFix(_PluginBase):
    ...
    @classmethod
    def _is_path_under_dirs(cls, path, dirs) -> bool:
        """
        判断 path 是否等于 dirs 中任一目录、或位于其下。

        同时用 commonpath 与规范化前缀两种方式判断，避免调用方写错参数顺序
        （_is_same_or_child_path 的 path/base 顺序）导致误判为“不属于”。
        """
        if not dirs:
            return False
        normalized_path = cls._normalize_config_path(str(path))
        for d in dirs:
            normalized_dir = cls._normalize_config_path(str(d))
            ...
            try:
                ...
                pass
            if normalized_path == normalized_dir or normalized_path.startswith(
                normalized_dir.rstrip("/\\") + os.sep
            ):
                return True
        return False
    ...
    def handle_directory_deleted(self, dir_path: Path):
        """
        处理目录删除事件。

        某些文件系统/媒体服务器在删除整个媒体目录时只产生目录级
        DELETE 事件，目录内的文件级 DELETE 事件可能不会被 watchdog
        观察到。由于 file_state 保存了监控文件的 dev/inode 信息，
        可以在目录已经不存在的情况下，根据历史路径找出该目录下
        已消失的文件，并复用 handle_deleted() 的完整清理流程。
        """
        try:
            ...
            with state_lock:
                deleted_paths = [
                    Path(path)
                    for path in list(self.file_state.keys())
                    if self._is_same_or_child_path(Path(path), str(dir_path))
                ]

            if not deleted_paths:
                logger.debug(f"目录删除未找到待处理文件记录：{dir_path}")
                return

            logger.info(
                f"目录删除检测到 {len(deleted_paths)} 个监控文件，"
                f"开始按文件删除处理：{dir_path}"
            )

            # 复用现有 handle_deleted()，这样立即删除/延迟删除、inode
            # 匹配、重新硬链接保护、刮削文件、转移记录、种子联动等逻辑
            # 都保持与正常文件删除完全一致。
    ...
    def _process_strm_pending(self):
        ...
        for f in files:
            fpath = Path(f)
            if self._is_path_under_dirs(fpath, deleted_top_dirs):
                skipped += 1
    ...
    def _cleanup_empty_dirs_upward(self, storage_type: str, start_dir: str):
        """
        整目录删除成功后的父目录兜底清理（轻量版）。

        网盘上的 start_dir 已被整目录删除，因此这里直接从它的**父目录**开始，
        自底向上检查是否因为这次删除而变空：空则删除并继续向上，非空立即停止，
        最多向上 3 级、全程只走 1 轮（不再多轮重试 + sleep）。

        旧实现会从 start_dir 起做 3 轮全量扫描（每级 get_file_item + list_files，
        轮间还 sleep 1 秒），一次整目录删除会额外产生十余次 115/CloudDrive2 请求，
        批量删剧时容易触发限流。整目录删除是原子操作、父目录列表不会滞后，
        所以单轮检查已经足够。
        """
        current = str(Path(start_dir.rstrip("/\\")).parent)

        for _ in range(3):
            if not current or current in ("/", "\\"):
                break

            cur_item = self._get_storage_dir_item(storage_type, current)
            if not cur_item:
                # 父目录已不存在 / 取不到目录项，停止
                break
            ...
            try:
                ...
                logger.debug(f"列出网盘目录失败: [{storage_type}] {current} - {ex}")
                break

            if files:
                # 仍含其它剧集/文件，停止向上，绝不越界删除
                break

            if not self._delete_storage_empty_dir(storage_type, cur_item):
                break

            logger.info(f"删除网盘空目录: [{storage_type}] {current}")
        ...
        logger.debug(
            f"STRM 整目录删除后的父目录清理完成: [{storage_type}] {start_dir}"
        )
