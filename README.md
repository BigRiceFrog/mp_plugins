...
**新增：对账扫描（事件丢失兜底）**，默认开启，间隔可配（默认 1800 秒）。插件原版完全依赖文件系统事件来发现删除，而 inotify 的 watch 是**按目录逐个**挂载的，以下场景事件根本不会送达，表现为"在 Jellyfin 里删了片子，插件毫无反应、日志一行都没有"：

- 新建目录时"目录创建"事件丢失或晚于其中的文件创建 → 该子树从此脱管；
- `fs.inotify.max_user_watches` 打满 → 之后新建的目录挂不上 watch；
- 插件重载 / 容器重启期间发生的创建与删除；
- 媒体库在容器挂载卷 / 网络盘 / FUSE 挂载上，宿主机进程（如 macOS 原生 Jellyfin）做的删除不一定能传成容器内事件。

这类问题是否命中取决于目录创建时序，所以表现为**间歇性、看起来没有规律**。对账扫描不依赖事件：周期性重扫监控目录并与 `file_state` 对账，目录里已消失的路径按文件删除处理（硬链接走 `handle_deleted`，STRM 走既有聚合缓冲），新出现的路径补入监控。事件路径保持原样，对账只在事件丢失时补上，最坏影响是响应延迟一个扫描间隔。

> 扫描只列目录项、不逐个 `stat`（`dev/inode` 仅对新出现的路径获取），因此在 USB / 网盘 / virtiofs 挂载上的代价约等于一次目录树遍历。媒体库很大或不希望频繁吵醒外接盘时，把间隔调大（如 3600 秒）或直接关闭该开关。
>
> 护栏：监控根目录当前不可用、或整个根目录读空但仍存有监控记录时，跳过该根目录不下删除结论；删除仍走延迟删除队列且执行前二次确认文件不存在，挂载抖动 / U 盘休眠不会误删文件。

**手动触发一次对账**（测试用，不必等间隔）：

- 配置页「对账扫描」区块里的 **「立即执行对账扫描」按钮** —— 点一下马上跑一轮，结果直接显示在按钮下方：扫描了几个目录 / 多少个文件、补录数、发现已删除数、待清理队列数、耗时、被跳过的不可用目录；
- 或在 MoviePilot 聊天框输入 `/removelink_scan`（插件命令）；
- 或调用接口 `GET /api/v1/plugin/RemoveLinkJellyfinFix/scan`。

三种入口走同一段逻辑，结果同时写入业务日志并推送一条通知。按钮的请求由宿主注入的 `window.MoviePilotAPI` 发出（自动带登录态），不需要为插件单独构建前端。**插件命令与 API 路由都要在 MoviePilot 重启后才注册**，首次使用请先重启一次；只想临时验证时，也可以把扫描间隔改成最小值 60 秒。

**排查提示**：`未在监控列表中，跳过处理` 已从 debug 提升到 info。日志里**完全没有**删除相关记录 → 事件没送达（对账扫描会在下一轮补上）；有"监测到删除文件"但没删任何东西 → 状态表缺记录或命中排除规则，看上面这行日志即可区分。
