...
| 插件 | 版本 | 说明 | 标签 |
| --- | --- | --- | --- |
| 清理媒体文件（Jellyfin修复版） | v2.17.0 | 修复 Jellyfin 删除媒体目录时硬链接未同步清理，并新增对账扫描兜底文件系统事件丢失 | 文件整理,媒体库,Jellyfin,硬链接 |
...
- 在 MoviePilot 聊天框输入 `/removelink_scan`（插件命令）；
- 或调用接口 `GET /api/v1/plugin/RemoveLinkJellyfinFix/scan`。

两者都会立刻执行一轮扫描，把「扫描目录数 / 文件数 / 补录数 / 发现已删除数 / 待清理队列数 / 耗时」写入业务日志并推送一条通知。插件命令与 API 路由都在 **MoviePilot 重启后**才会注册；只想临时验证时，也可以把扫描间隔临时改成最小值 60 秒。
