...
| 下载器流量统计 | v1.0.0 | 按年/月/日统计下载器上传/下载流量并细分到每个 PT 站点；月度上传超阈值自动全局限速、月初按 Cron 自动恢复 | 下载器,流量统计,qBittorrent,Transmission,PT |
...
**配置项**：采集周期（Cron，默认每 30 分钟）、生效下载器（MP 已配置中多选）、月度上传阈值 (GB)、超限后全局上传 / 下载限速 (KB/s)、月初恢复全局上传 / 下载限速 (KB/s)、月初恢复触发时间 (Cron)。
